...
| `NodeLocalFabricIPC` | Alpha | `false` | See [Node-local fabric IPC](../prerequisites.md#node-local-fabric-ipc). | Starts a single-node IMEX service and exposes the allocated channel to ComputeDomain workloads on supported nodes that do not belong to an MNNVL clique. This enables node-local CUDA fabric-handle exchange across isolated pods. |
