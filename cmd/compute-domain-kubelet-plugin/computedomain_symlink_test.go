...
// The config root is created here as well, and a cold start is the case where
// it is not there yet.
func TestWriteConfigFileCreatesTheConfigRoot(t *testing.T) {
	template := filepath.Join(t.TempDir(), "template.cfg")
	require.NoError(t, os.WriteFile(template, []byte("template contents"), 0644))

	configRoot := filepath.Join(t.TempDir(), "driver", "domains")
	require.NoDirExists(t, configRoot)
	...
	got, err := os.ReadFile(filepath.Join(configRoot, domainID, "imexd.cfg.tmpl"))
	require.NoError(t, err)
	require.Equal(t, "template contents", string(got))
}

// Reading the template is the one step that happens before any directory is
// created, so a template that is not there leaves the config root as it was.
func TestWriteConfigFileLeavesNoDomainDirectoryWhenTheTemplateIsUnreadable(t *testing.T) {
	configRoot := t.TempDir()
	const domainID = "d3b07384-d9a7-4e2b-8f1a-2c1e6b5a9f00"
	...
	settings, err := (&ComputeDomainManager{configFilesRoot: configRoot}).NewSettings(domainID)
	require.NoError(t, err)
	settings.templateSourcePath = filepath.Join(t.TempDir(), "not-there.cfg")

	require.ErrorContains(t, settings.WriteConfigFile(context.Background()), "error reading template file")

	require.NoDirExists(t, filepath.Join(configRoot, domainID))
	untouched, err := os.ReadFile(other)
	require.NoError(t, err)
	require.Equal(t, "someone else", string(untouched))
}
